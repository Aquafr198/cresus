...
pub async fn upload_metadata(
    ...
    // Cap at 10 MB — anything bigger is unlikely to be a legit token logo
    // and would just burn upload time before Pump.fun rejects it.
    if !config.image_url.is_empty() {
        const MAX_IMG_BYTES: usize = 10 * 1024 * 1024;
        ...
        let resp = client
            ...
            .await
            .map_err(|e| {
                PumpFunError::MetadataUploadFailed(format!("Failed to fetch image: {}", e))
            })?;

        // Fail fast on a Content-Length header that already exceeds the cap.
        if let Some(declared) = resp.content_length() {
            if declared > MAX_IMG_BYTES as u64 {
                return Err(PumpFunError::MetadataUploadFailed(format!(
                    "image too large: {} bytes (max {} = 10 MB)",
                    declared, MAX_IMG_BYTES
                )));
            }
        }

        let img_bytes = resp.bytes().await.map_err(|e| {
            PumpFunError::MetadataUploadFailed(format!("Failed to read image: {}", e))
        })?;

        // Belt + suspenders: some servers stream without Content-Length.
        if img_bytes.len() > MAX_IMG_BYTES {
            return Err(PumpFunError::MetadataUploadFailed(format!(
                "image too large: {} bytes (max {} = 10 MB)",
                img_bytes.len(),
                MAX_IMG_BYTES
...
/// Live state of a Pump.fun bonding curve, fetched on-demand.
///
/// `fill_pct` is computed from `real_sol_reserves` against the canonical
/// graduation threshold (85 SOL ≈ 85_000_000_000 lamports as of 2024). When
/// `graduated == true`, liquidity has migrated to Raydium and the curve is
/// no longer the active trading venue.
...
/// Graduation threshold in lamports. Sourced from the Pump.fun program's
/// onchain global config (85 SOL). When `real_sol_reserves` crosses this,
/// the curve auto-migrates to Raydium.
const PUMP_FUN_GRADUATION_LAMPORTS: u64 = 85_000_000_000;

/// Fetch the live bonding-curve state for a mint. Returns `None` if the
/// PDA doesn't exist (mint was never a Pump.fun launch, or already
/// graduated and the account was closed).
pub async fn get_curve_state(
    mint: &Pubkey,
    rpc_client: &solana_client::nonblocking::rpc_client::RpcClient,
) -> Result<Option<BondingCurveState>, PumpFunError> {
    let (pda, _) = derive_bonding_curve(mint);

    let account = match rpc_client.get_account(&pda).await {
        Ok(acc) => acc,
        Err(e) => {
            let msg = e.to_string();
            ...
            return Err(PumpFunError::RpcError(msg));
        }
    };
    ...
    //   complete:               bool (u8)
    if account.data.len() < 8 + 8 * 5 + 1 {
        return Err(PumpFunError::Other(format!(
    ...
    let read_u64 = |off: usize| -> u64 {
        let mut b = [0u8; 8];
        b.copy_from_slice(&account.data[off..off + 8]);
        u64::from_le_bytes(b)
    };

    let real_token_reserves = read_u64(8 + 16);
    let real_sol_reserves = read_u64(8 + 24);
    let complete = account.data[8 + 40] != 0;

    let fill_pct = if complete {
        100.0
    } else {
        ((real_sol_reserves as f64) / (PUMP_FUN_GRADUATION_LAMPORTS as f64) * 100.0).min(100.0)
    };

    Ok(Some(BondingCurveState {
        fill_pct,
        sol_in_curve: real_sol_reserves as f64 / 1_000_000_000.0,
        tokens_remaining: real_token_reserves,
        graduated: complete,
    }))
}
...
pub async fn create_pump_fun_token(
    ...
    let priority_fee = std::env::var("OFFIVEX_PRIORITY_FEE_MICRO_LAMPORTS")
        .ok()
