//! Integration tests for cresus-core
//!
...
async fn setup_test_db() -> Connection {
    let conn = cresus_db::init_db(":memory:").await.unwrap();
    conn
}
...
mod wallet_tests {
    ...
    async fn test_create_wallet_and_decrypt_keypair() {
        ...
        let keypair = cresus_core::wallet::decrypt::decrypt_wallet_keypair(
            &db,
            &wallet.id,
            &mek,
        )
...
mod encryption_tests {
    use super::*;
    use cresus_core::wallet::encryption;
    use cresus_core::wallet::keygen;
...
mod error_tests {
    use cresus_core::errors::{CategorizedError, ErrorCategory};
    use cresus_core::trading::swap::SwapError;
    use cresus_core::trading::jupiter::JupiterError;
...
mod volume_bot_tests {
    use super::*;
    use cresus_core::trading::volume_bot::{VolumeBot, VolumeBotConfig};
...
mod warmer_tests {
    use super::*;
    use cresus_core::wallet::warmer::{WalletWarmer, WarmerConfig};
...
mod backup_tests {
    use cresus_db::backup::{backup_database, BackupConfig, list_backups};
...
// ─── Validation Tests ────────────────────────────────────────────────────
// Note: cresus_api is not a dependency of cresus-core, so we test
// validation logic that lives in cresus-api through its own test module.
// Here we validate the password rules that the frontend/backend must agree on.

mod validation_tests {
    /// Mirrors the backend validate_password logic (cresus_api::validation)
...
mod pump_fun_tests {
    use cresus_core::trading::pump_fun::PumpFunLaunchConfig;
    use cresus_core::errors::CategorizedError;

    #[test]
    fn test_pump_fun_error_categories() {
        use cresus_core::trading::pump_fun::PumpFunError;
        use cresus_core::errors::ErrorCategory;
...
mod devnet_tests {
    ...
    async fn devnet_wallet_create_and_check_balance() {
        ...
        let db = Arc::new(db);
        let master_key = Arc::new(RwLock::new(None));
        let mgr = cresus_core::wallet::manager::WalletManager::new(db.clone(), master_key.clone());
    ...
    async fn devnet_rpc_manager_with_endpoint() {
        ...
        let db = setup_test_db().await;
        let db = Arc::new(db);
        let rpc_mgr = cresus_core::rpc::manager::RpcManager::new(db.clone());
