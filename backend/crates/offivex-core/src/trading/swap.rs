...
/// Sell a percentage of a wallet's available token balance.
///
/// Computes `amount = available_amount × percent / 100` (saturating), then
/// delegates to `swap_token_to_sol`. Used by the quick-sell keybind feature
/// which has already fetched per-wallet balances upstream.
///
/// `percent` is clamped to 1..=100. A zero amount returns `InsufficientBalance`
/// rather than submitting a no-op swap.
pub async fn sell_partial_balance(
    wallet: &Keypair,
    token_mint: &Pubkey,
    ...
    let amount = ((available_amount as u128).saturating_mul(pct) / 100) as u64;
    if amount == 0 {
        return Err(SwapError::InsufficientBalance(format!(
            "computed sell amount is zero (available={}, percent={})",
            available_amount, percent
        )));
    }
    swap_token_to_sol(wallet, token_mint, amount, slippage_bps, rpc_client).await
}
