...
fn read_u64(data: &[u8], offset: usize) -> Result<u64, FeeError> {
    let slice = data
        .get(offset..offset + 8)
        .ok_or(FeeError::PoolNotFound)?;
    // Safe: .get(offset..offset+8) returns exactly 8 bytes if Some.
    Ok(u64::from_le_bytes(slice.try_into().expect("slice is exactly 8 bytes")))
}

fn read_pubkey(data: &[u8], offset: usize) -> Result<Pubkey, FeeError> {
    let slice = data
...
pub async fn get_unclaimed_fees(
    ...
    // Bounds-checked: returns FeeError::PoolNotFound rather than panicking
    // if the on-chain account has been corrupted or truncated.
    ...
    // Integer math (u128 intermediate) to avoid f64 rounding losses against the
    // creator. For SPL token amounts (max ~10^19), u128 multiplication never
    // overflows.
    let (token_fees, sol_fees) = if lp_supply > 0 && lp_balance > 0 {
        let lp_balance_u128 = lp_balance as u128;
        let lp_supply_u128 = lp_supply as u128;
        (
            (need_take_pnl_coin as u128 * lp_balance_u128 / lp_supply_u128) as u64,
            (need_take_pnl_pc as u128 * lp_balance_u128 / lp_supply_u128) as u64,
        )
    } else {
        (0, 0)
    };
