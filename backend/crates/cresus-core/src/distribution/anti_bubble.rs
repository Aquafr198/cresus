...
pub fn split_amounts(total_lamports: u64, n: usize, variation: &AmountVariation) -> Vec<u64> {
    ...
    let base_amount = total_lamports as f64 / n as f64;
    ...
    let weights: Vec<f64> = (0..n)
        .map(|_| {
            let deviation = rng.gen_range(-variation.max_deviation_pct..=variation.max_deviation_pct);
            (1.0 + deviation).max(0.1)
        })
        .collect();

    let weight_sum: f64 = weights.iter().sum();
    let scale = total_lamports as f64 / (base_amount * weight_sum);

    let mut amounts: Vec<u64> = weights
        .iter()
        .map(|w| (base_amount * w * scale).round() as u64)
        .collect();

    let sum: u64 = amounts.iter().sum();
    if sum > total_lamports {
        amounts[n - 1] = amounts[n - 1].saturating_sub(sum - total_lamports);
    } else {
        amounts[n - 1] += total_lamports - sum;
    }
