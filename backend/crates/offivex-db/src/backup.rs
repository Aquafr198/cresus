...
pub async fn backup_database(
    ...
    let backup_filename = format!("OFFIVEX_{}.db", timestamp);
...
pub struct OffSiteBackupConfig {
    ...
    /// Examples:
    ///   "aws s3 cp {file} s3://my-bucket/Offivex-backups/"
    ///   "rclone copy {file} remote:Offivex-backups/"
    ///   "mc cp {file} myminio/Offivex-backups/"
    pub upload_command: String,
}

impl OffSiteBackupConfig {
    /// Create from the `OFFIVEX_BACKUP_UPLOAD_CMD` environment variable.
    /// Returns None if the env var is not set.
    pub fn from_env() -> Option<Self> {
        std::env::var("OFFIVEX_BACKUP_UPLOAD_CMD")
            .ok()
            .filter(|s| !s.is_empty())
...
pub async fn upload_backup_offsite(
    ...
    // Audit L.2 — POSIX-safe shell escape before string-substituting into
    // the configured command. The backup filename is generated by us
    // (deterministic format), but defense in depth: if a future change
    // ever lets a user influence the filename, this prevents command
    // injection via `;`, `$(...)`, backticks etc. The single-quote
    // wrapping is the canonical POSIX-safe escape.
    let quoted_path = shell_single_quote(path_str);
    let command = config.upload_command.replace("{file}", &quoted_path);
...
/// POSIX-safe shell escape: wrap in single quotes, escape any embedded
/// single quote as `'\''` (the canonical bash/sh-portable form). Result is
/// suitable for direct substitution into a `sh -c "..."` command.
///
/// Examples:
///   `foo` → `'foo'`
///   `it's` → `'it'\''s'`
///   `; rm -rf /` → `'; rm -rf /'`  (the quotes neutralise the metachars)
