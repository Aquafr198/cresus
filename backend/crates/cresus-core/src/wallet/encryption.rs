use cresus_crypto::{encrypt, decrypt, EncryptedPayload, SecretBytes};

/// Encrypt a wallet's secret key bytes using the Master Encryption Key.
pub fn encrypt_secret_key(
    secret_bytes: &[u8],
    master_key: &SecretBytes,
) -> Result<EncryptedPayload, cresus_crypto::aes::CryptoError> {
...
pub fn decrypt_secret_key(
    ...
    master_key: &SecretBytes,
) -> Result<SecretBytes, cresus_crypto::aes::CryptoError> {
